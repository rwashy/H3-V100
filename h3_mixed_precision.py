...
from https://github.com/Icbears/minimax-h3-v100-patch. Unlike the original
file patcher, this module applies the replacement to a cloned ComfyUI MODEL
via object patches.
"""
...
def _unwrap_our_block_forward(value):
    ...
    while current is not None:
        ...
        if identity in seen:
            raise RuntimeError("H3 block patch detected a cyclic wrapper chain.")
        seen.add(identity)
        current = getattr(function, BLOCK_ORIGINAL_FORWARD_ATTR, None)
    raise RuntimeError("H3 block patch could not recover its original forward.")
...
def _chunked_qk_rope(self, q, k, rope_freqs, transformer_options):
    ...
    if key not in _reported_shapes:
        _reported_shapes.add(key)
        LOGGER.info(
...
def _trim_and_log(device, transformer_options, reason):
    ...
    if trimmed is not None:
        LOGGER.info(
            "V100 adaptive cache trim: cuda_free_before/after=%.1f/%.1f MiB, "
...
def _qkv_projection(self, proj_x, transformer_options):
    ...
    if key not in _reported_shapes:
        ...
        if trimmed is not None:
            LOGGER.info(
                "V100 adaptive QKV cache trim: cuda_free_before/after=%.1f/%.1f MiB, "
...
def _out_projection(self, out, transformer_options):
    ...
    if extreme:
        chunk_tokens = min(chunk_tokens, extreme_chunk_tokens)
    # The attention output projection must remain on its validated FP32 boundary.
    def project(part):
...
def h3_v100_attention_forward(self, x, rope_freqs=None, transformer_options={}):
    ...
    return _out_projection(self, out.squeeze(0), transformer_options)
...
def _make_h3_block_forward(original_forward):
...
class H3V100MixedPrecision:
    ...
    CATEGORY = "V100"
    DESCRIPTION = (
        "V100 Plan 2 mixed precision: FP16 QKV and attention; FP32 Q/K norm, "
        "RoPE, output projection, MLP and residual stream. Compatible with "
        "workflow-scoped TE-Speed model wrappers."
    )
    ...
    def patch(self, model, enabled=True, v100_only=True):
        ...
        for index, block in enumerate(blocks):
            ...
            patched_model.add_object_patch(
                block_key,
                types.MethodType(
                    _make_h3_block_forward(base_block_forward), block
                ),
            )
