...
#ifdef COMFY_V100_HDIM128_256_ONLY
    TORCH_CHECK(params.d == 128 || params.d == 256,
                "comfy_v100_flash_attn only supports head_dim=128 or head_dim=256");
    TORCH_CHECK(!params.is_causal, "comfy_v100_flash_attn only supports non-causal attention");
    TORCH_CHECK(params.num_splits <= 1 && !force_split_kernel,
                "comfy_v100_flash_attn does not support split-KV or paged KV");
    if (params.d == 128) {
        run_mha_fwd_<128, false>(params, stream);
    } else {
        run_mha_fwd_<256, false>(params, stream);
    }
#else
