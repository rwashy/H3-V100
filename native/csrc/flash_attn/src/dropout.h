...
struct Dropout {
    ...
    __forceinline__ __device__ void apply_dropout(Tensor<Engine, Layout> &tensor_,
        ...
        // This inference-only build always instantiates the kernel with
        // Is_dropout=false. Keep the member callable so CUDA can parse the
        // discarded branch without instantiating unsupported layout helpers.
        (void)tensor_;
        (void)block_row_start;
        (void)block_col_start;
